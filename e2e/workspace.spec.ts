...
test('shows translation markup help and renders semantic translation styles', async ({ page }) => {
  ...
  await expect(help.locator('dt').filter({ hasText: '強調' })).toBeVisible()
  await expect(help.getByText(/原文または訳文に次の記法/)).toBeVisible()
  await expect(help.getByText('|漢字《かんじ》', { exact: true })).toBeVisible()
  await help.getByRole('button', { name: '訳文の記法を閉じる' }).click()

  const source = page.getByRole('textbox', { name: '翻訳する原文' })
  await source.fill('**太字原文** _強調原文_ ~取消原文~ ｜漢字《かんじ》')
  await source.evaluate((element: HTMLTextAreaElement) => {
    element.focus()
    element.setSelectionRange(0, element.value.length)
  })
  await page.getByRole('button', { name: '選択範囲を翻訳 →' }).click()
  await page.getByRole('textbox', { name: '訳文' }).fill('**太字** _強調_ ~取消~ ｜漢字《かんじ》')
  await page.getByRole('button', { name: '訳文を登録 ⌘↵' }).click()

  const registered = page.locator('.pair-translation')
  await expect(registered.locator('strong')).toHaveText('太字')
  await expect(registered.locator('em')).toHaveText('強調')
  await expect(registered.locator('em')).toHaveCSS('font-style', 'italic')
  ...
  const registeredSource = page.locator('.pair-source')
  await expect(registeredSource.locator('strong')).toHaveText('太字原文')
  await expect(registeredSource.locator('em')).toHaveText('強調原文')
  await expect(registeredSource.locator('em')).toHaveCSS('font-style', 'italic')
  await expect(registeredSource.locator('s')).toHaveText('取消原文')
  await expect(registeredSource.locator('ruby')).toHaveText('漢字かんじ')

  await page.getByRole('button', { name: '閲覧' }).click()
  const readerTranslation = page.locator('.reader-translation')
  await expect(readerTranslation.locator('strong')).toHaveText('太字')
  await expect(readerTranslation.locator('em')).toHaveText('強調')
  await expect(readerTranslation.locator('em')).toHaveCSS('font-style', 'italic')
  await expect(readerTranslation.locator('s')).toHaveText('取消')
  await expect(readerTranslation.locator('rt')).toHaveText('かんじ')
  const readerSource = page.locator('.reader-source')
  await expect(readerSource.locator('strong')).toHaveText('太字原文')
  await expect(readerSource.locator('em')).toHaveText('強調原文')
  await expect(readerSource.locator('em')).toHaveCSS('font-style', 'italic')
  await expect(readerSource.locator('s')).toHaveText('取消原文')
  await expect(readerSource.locator('ruby')).toHaveText('漢字かんじ')
})
